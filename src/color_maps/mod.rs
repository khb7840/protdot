...
pub fn color_for_chain(chain: &str, color_maps: &ColorMaps) -> Color {
    seeded_random_color(chain_seed(chain), color_maps)
}

fn chain_seed(chain: &str) -> u32 {
    chain.bytes()
        .fold(0u32, |seed, byte| seed.wrapping_mul(37).wrapping_add(byte as u32 + 1))
        .max(1)
}
