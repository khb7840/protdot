...
impl ColorPickerState {
    ...
    pub fn get_items(&self) -> Vec<String> {
        match self.category {
            ...
            1 => vec!["C".to_string(), "O".to_string(), "N".to_string(), "S".to_string(), "H".to_string(), "P".to_string()],
            2 => vec!["Hydrophobic".to_string(), "Polar".to_string(), "Positive".to_string(), "Negative".to_string(), "Glycine".to_string()],
            _ => vec![],
        }
    }
...
pub fn draw_color_picker(
    ...
    draw_rectangle(panel_x, panel_y, panel_w, panel_h, Color::new(0.2, 0.2, 0.2, 0.9));
    draw_rectangle_lines(panel_x, panel_y, panel_w, panel_h, 2.0, WHITE);
    
    draw_text("Color Settings (P to close)", panel_x + 10.0, panel_y + 25.0, 20.0, WHITE);
    
    let categories = ["Background", "Elements", "AA Groups"];
    let category_name = categories[picker_state.category];
    draw_text(&format!("Category (Tab): {}", category_name), panel_x + 10.0, panel_y + 55.0, 18.0, YELLOW);
