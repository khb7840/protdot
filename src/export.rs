...
fn render_svg_per_atom(
    ...
    for (atom, screen_pos, depth) in projected_atoms.iter() {
        ...
        writeln!(file, r#"    <circle cx="{:.2}" cy="{:.2}" r="{:.2}" fill="rgb({},{},{})" fill-opacity="1.0" />"#,
...
fn render_svg_per_residue(
    ...
    for atom in atoms {
        residues.entry((atom.chain.clone(), atom.residue_num)).or_insert_with(Vec::new).push(atom);
    }
    
    let min_res = residues.keys()
        .map(|(_, res_num)| *res_num)
        .min()
        .unwrap_or(0) as f32;
    let max_res = residues.keys()
        .map(|(_, res_num)| *res_num)
        .max()
        .unwrap_or(1) as f32;
    ...
    for ((chain_id, res_num), res_atoms) in residues.iter() {
        ...
        residue_data.push(((chain_id.clone(), *res_num), screen_pos, depth, sphere_radius, color));
    }
