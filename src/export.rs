...
fn render_svg_per_atom(
    ...
    let mut projected_atoms: Vec<_> = atoms
        .iter()
        .map(|atom| {
            let transformed_pos = apply_transform(atom.position, rotation, translation);
            let screen_pos =
                project_to_screen(transformed_pos, camera, screen_width, screen_height);
    ...
    for (atom, screen_pos, depth) in projected_atoms.iter() {
        ...
        writeln!(
            file,
            r#"    <circle cx="{:.2}" cy="{:.2}" r="{:.2}" fill="rgb({},{},{})" fill-opacity="1.0" />"#,
            screen_pos.x,
            screen_pos.y,
            radius,
            (color.r * 255.0) as u8,
...
fn project_to_screen(pos: Vec3, camera: &Camera3D, screen_width: f32, screen_height: f32) -> Vec2 {
...
fn get_color(
    ...
    match color_scheme {
        ...
        ColorScheme::ByAminoAcidGroup => {
            Atom::get_color_by_amino_acid_group(&atom.residue, color_maps)
        }
        ColorScheme::ByAminoAcidType => {
            Atom::get_color_by_amino_acid_type(&atom.residue, color_maps)
        }
